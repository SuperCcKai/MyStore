...
<script type="text/javascript">
	function confirmReceive(orderNo){
		if(confirm("确认收货？")){
			$.post("order_confirmReceive.action", {status:4, orderNo:orderNo},
				function(result){
...
<h2 style="margin-top: 0px;">订单管理</h2>
	<div class="manage" style="padding-top: 0px;">
		<div class="search">
		...
		<table class="list" style="margin-top: 20px;">
			<c:forEach items="${orderList }" var="order">
				<tr style="background-color: #f7f4eb">
					<td colspan="4">
						...
						状态：<c:choose>
								...
								<c:when test="${order.status == 3 }"><input type="button" value="确认收货" style="border: 1px solid red" onclick="confirmReceive(${order.orderNo })"/></c:when>
				...
				<c:forEach items="${order.orderProductList }" var="orderProduct">
					<tr>
						<td width="50%">
							<a href="product_showProduct.action?productId=${orderProduct.product.id }" target="_blank"><img width="72" height="72" title="${orderProduct.product.name }" src="${orderProduct.product.proPic }"></a>
							&nbsp;&nbsp;
