...
<script type="text/javascript">
	...
	function deleteProductSmallType(){
		...
		$.messager.confirm("系统提示：", "您确认要删除这<font color=red>"+selectedRows.length+"</font>条数据吗？", function(r){
			if(r){
				$.post("productSmallType_delete.action", {ids:ids}, 
					function(result){
						var result = eval('('+result+')');
						if(result.success){
							if(result.exist){
	...
	function closeProductSmallTyperDialog(){
		$("#dlg").dialog("close");
		resetValue();//重置表单的操作 
	}
...
<body style="margin: 1px">
	<table id="dg" title="商品小类管理" class="easyui-datagrid" 
	 ...
	 <div id="dlg-buttons">
	 	...
	 	<a href="javascript:closeProductSmallTyperDialog()" class="easyui-linkbutton" iconCls="icon-cancel">关闭</a>
	 </div>
</body>
</html>