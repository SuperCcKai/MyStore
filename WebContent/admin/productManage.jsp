...
<script type="text/javascript">
	...
	function setProductHot(){
		var selectedRows = $("#dg").datagrid('getSelections');
		if(selectedRows.length == 0){
			$.messager.alert("系统提示", "请选择要设置为热卖的商品！");
			return;
		}
		var strIds = [];
		for(var i = 0; i<selectedRows.length; i++){
		...
		$.messager.confirm("系统提示：", "您确认要设置这<font color=red>"+selectedRows.length+"</font>个商品为热卖吗？", function(r){
			if(r){
				$.post("product_setProductWithHot.action", {ids:ids}, 
					function(result){
						var result = eval('('+result+')');
						if(result.success){
							$.messager.alert("系统提示", "商品已设置成功！");
							$("#dg").datagrid("reload");
						}else{
							$.messager.alert("系统提示", "商品设置失败！");
						}
					}
				);
			}
	...
	function setProductSpecialPrice(){
		var selectedRows = $("#dg").datagrid('getSelections');
		if(selectedRows.length == 0){
			$.messager.alert("系统提示", "请选择要设置为特价的商品！");
			return;
		}
		var strIds = [];
		for(var i = 0; i<selectedRows.length; i++){
		...
		$.messager.confirm("系统提示：", "您确认要设置这<font color=red>"+selectedRows.length+"</font>个商品为特价吗？", function(r){
			if(r){
				$.post("product_setProductWithSpecialPrice.action", {ids:ids}, 
					function(result){
						var result = eval('('+result+')');
						if(result.success){
							$.messager.alert("系统提示", "商品已设置成功！");
	 ...
	 url="product_list.action" fit="true" toolbar="#tb">
	 	<thead>
	 		<tr>
	 			...
	 			<th field="hot" width="50" align="center" formatter="formatHot">是否特卖</th>
	 	...
	 	<form id="fm" method="post" enctype="multipart/form-data">
	 		<table cellspacing="8px">
	 			...
	 			<tr>
	 				<td>所属小类：</td>
	 				<td colspan="4">
	 					<input class="easyui-combobox" id="sName" name="product.smallType.id" data-options="panelHeight:'150', editable:false, valueField:'id', textField:'name', url:'productSmallType_comboList.action'"/>
	 				</td>
	 			</tr>
	 			<tr>
	 				<td valign="top">备注：</td>
