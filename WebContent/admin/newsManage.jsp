...
<script type="text/javascript">
	...
	function openNewsModifyDialog(){
		...
		if(selectedRows.length != 1){
			$.messager.alert("系统提示", "请选择一条要编辑的新闻！");
			return;
		}
	...
	function deleteNews(){
		...
		var ids = strIds.join(",");
		$.messager.confirm("系统提示：", "您确认要删除这<font color=red>"+selectedRows.length+"</font>条新闻吗？", function(r){
			if(r){
				$.post("news_delete.action", {ids:ids}, 
					function(result){
						var result = eval('('+result+')');
						if(result.success){
							$.messager.alert("系统提示", "新闻已成功删除！");
							$("#dg").datagrid("reload");
						}else{
							$.messager.alert("系统提示", "新闻删除失败！");
						}
					}
				);
			}
