...
<script type="text/javascript">
	...
	function openNoticeModifyDialog(){
		...
		if(selectedRows.length != 1){
			$.messager.alert("系统提示", "请选择一条要编辑的公告！");
			return;
		}
	...
	function deleteNotice(){
		...
		var ids = strIds.join(",");
		$.messager.confirm("系统提示：", "您确认要删除这<font color=red>"+selectedRows.length+"</font>条公告吗？", function(r){
			if(r){
				$.post("notice_delete.action", {ids:ids}, 
					function(result){
						var result = eval('('+result+')');
						if(result.success){
							$.messager.alert("系统提示", "公告已成功删除！");
							$("#dg").datagrid("reload");
						}else{
							$.messager.alert("系统提示", "公告删除失败！");
						}
					}
				);
			}
