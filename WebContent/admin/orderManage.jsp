...
<script type="text/javascript">
	...
	function modifyOrderStatus(status){
		...
		$.messager.confirm("系统提示：", "您确认要处理这<font color=red>"+selectedRows.length+"</font>条订单吗？", function(r){
			if(r){
				$.post("order_modifyOrderStatus.action", {orderNos:orderNos,status:status}, 
					function(result){
						var result = eval('('+result+')');
						if(result.success){
							$.messager.alert("系统提示", "订单已成功处理！");
							$("#dg").datagrid("reload");
						}else{
							$.messager.alert("系统提示", "订单处理删除失败！");
						}
					}
				);
			}
	 ...
	 url="order_list.action" fit="true" toolbar="#tb">
	  ...
	  <div id="tb">
	 	...
	 	<div>
	 		...
	 		&nbsp;订单人：&nbsp;<input type="text" id="s_userName" size="20" onkeydown="if(event.keyCode==13) searchOrder()"/>
	 		<a href="javascript:searchOrder()" class="easyui-linkbutton" iconCls="icon-search" plain="true">搜索</a>
	 	</div>
	 </div>
	 
	 <div id="dlg" class="easyui-dialog" style="width: 750px; height: 400px; padding: 10px 30px" closed="true" buttons="#dlg-buttons">
 		...
 		<table id="dg2" title="订单商品列表" class="easyui-datagrid" 
		 	...
		 	<thead>
		 		<tr>
		 			...
		 			<th field="num" width="50" align="center">商品购买数量</th>
